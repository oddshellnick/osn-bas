...
class ChromeFeatures(BlinkFeatures, total=False):
	pass
...
class ChromeWebDriver(BlinkWebDriver):
	...
	def __init__(
		...
		Args:
			...
			browser_exe (Optional[Union[str, pathlib.Path]]): Path to the Chrome Browser executable. If `use_browser_exe` is True and `browser_exe` is None, the path is automatically detected. If `use_browser_exe` is False, this parameter is ignored. Defaults to None.
			flags (Optional[ChromeFlags]): Specific Chrome Browser flags and options to apply. Defaults to None.
		...
		if browser_exe is None and use_browser_exe:
			browser_exe = get_path_to_browser("Chrome")
		...
		super().__init__(
				...
				flags_manager_type=ChromeFlagsManager,
				flags=flags,
				start_page_url=start_page_url,
